...
use super::{HasRenderer, HasSession, UpdateAndRender};
use crate::config::{Expression, ViewConfigUpdate};
use crate::utils::ApiFuture;

pub trait CreateColumn: HasSession + HasRenderer + UpdateAndRender {
    fn clone_column(&self, name: &str, in_place: bool, _open_settings: bool) -> ApiFuture<()> {
        // 1. Create a new expression.
        let expr_name = self.session().metadata().make_new_column_name(Some(name));
        let expr_val = format!("\"{name}\"");
        let expr = Expression::new(&expr_name, &expr_val);
        let view_config = self.session().get_view_config();
        let mut serde_exprs = view_config.expressions.clone();
        serde_exprs.retain(|name, _expr| name != &expr.name);
...
impl<T: HasSession + HasRenderer> CreateColumn for T {}
