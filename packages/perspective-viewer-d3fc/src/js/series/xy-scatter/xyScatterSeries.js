...
import { seriesLinearRange } from "../../series/seriesRange";
 ...
 */
export default function xyScatterSeries() {
    let settings = null;
    let data = null;
    let color = null;
    let symbols = null;

    const _xyScatterSeries = (container) => {
    ...
    _xyScatterSeries.settings = (...args) => {
        if (!args.length) {
            return settings;
        }
        settings = args[0];
